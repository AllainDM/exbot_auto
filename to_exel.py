...
filter_iss = [["Днепропетровская", "д37"], ["Есенина", "д1к1*"],
              ]
...
def save_all_to_exel(table_name, arr, t_o):
    ...
    for i in arr:
        print("Тут смотрим строчку для записи в ексель")
        print(i)
        for ii in i:
            print(ii)
            if_true = True
            for street_in_filter in filter_iss:
                # print(f"street_in_filter: {street_in_filter}")
                if street_in_filter[0] in ii[2] and street_in_filter[1] in ii[2]:
                    # print(ii[2])
                    if_true = False
                # else:
            if if_true:
    ...
    ws.write(3, 2, xlwt.Formula('SUM(J2:J7)+SUM(K10:K13)'))
