...
def save_all_to_exel(table_name, arr, t_o):
    ...
    # ЭтХоум
    ws.write(1, 8, "ЭтХоум")
    ws.write(2, 8, "Подключено")
    ws.write(2, 9, xlwt.Formula(
        'COUNTIF(AV2:AV1000;"Архив")+'
        'COUNTIF(AV2:AV1000;"Подключен")+'
        'COUNTIF(AV2:AV1000;"Тариф")'))
    ws.write(2, 10, "(Архив, Подключен, Тариф)")
    ...
    ws.write(4, 10, "(Тайм-аут, НаОбзвон, ПростоЗаявка, Дубль, Нестандарт, Неверный адрес, ЮрЛицо)")
